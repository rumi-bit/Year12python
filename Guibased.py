...
class AppWindow:
    def __init__(self, logic_ref):
        ...
        def savetext(self, font):
            self.textcache = {}
            for key, data in self.inputs.items():
                surface = TTF_RenderText_Blended(font, data["label"].encode("utf-8"), SDL_Color(255, 255, 255, 255))
                texture = SDL_CreateTextureFromSurface(self.render, surface)
                self.label_textures[key] = {"texture": texture, "w": surface.contents.w, "h": surface.contents.h}
    ...
    def drawtext(self, data, font_ref, color=(255, 255, 255)):
        r = data["rect"]
        if data["changed"]:
            if data["texture"]:
                SDL_DestroyTexture(data["texture"])
            if not data["text"] or not font_ref:
                return
            sdl_color = SDL_Color(color[0], color[1], color[2], 255)
            surface = TTF_RenderText_Blended(font_ref, data["text"].encode("utf-8"), sdl_color)
            if not surface:
                return

            texture = SDL_CreateTextureFromSurface(self.renderer, surface)
            w, h = surface.contents.w, surface.contents.h
            dest_rect = SDL_Rect(r.x, r.y, w, h)
    ...
    def draw_menu(self):
        self.drawtext("Freddy's Fast Fish", self.font_title)
    ...
    def details(self):
        ...
        for key, data in self.inputs.items():
            r = data["rect"]
            self.drawtext(data, self.font_main)
            ...
            if key == "name":
                    self.validator(data["text"], r.x, r.y, r.w, r.h, checktype=("alpha",), length=(1, 20), message="Invalid name", active=key)
            elif key == "phone":
                    self.validator(data["text"], r.x, r.y, r.w, r.h, checktype=("digit",), length=(1, 20), message="Invalid phonenumber", active=key)
